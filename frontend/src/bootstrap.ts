...
export function installRuntimeContracts(globalWindow: Window): SvelteUiGlobal | null {
  try {
    ...
    const api: SvelteUiGlobal = { UI_READY, mountSvelteUi, unmountSvelteUi, bridge, handshake: (request) => bridge.handshake(request) };
    globalWindow.KohakuLoomSvelteUi = api;
    return api;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    globalWindow.KohakuLoomSvelteUi = { UI_READY, mountSvelteUi, unmountSvelteUi, error: message };
